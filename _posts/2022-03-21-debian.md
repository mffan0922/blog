...
连通VPN后，发现公司的跳板机还是登不上去，但是其他VPS都没有问题，一番搜索之后定位是`mtu`的值导致的。VPN虚拟网卡的mtu默认值是1500，修改小一些就可以了，比如1300，Linux下配置一般都有两种修改方式：临时修改和永久修改。
...
> 使用`dpkg`命令安装软件包的时候，可能会遇到依赖问题，最简单的方法是执行`apt install -f`或者`apt --fix-broken install`，实在解决不了，那就只能一个个的去解决，或者放弃。
